...
// 3d目标信息获取=====
    ...
    for( int m=0; m<objnumber; m++)
    {
      ...
      std::cout<< "obj: " << cluster.class_id << " " << cluster.prob << " "
               << cent[0] << " " << cent[1] << " " << cent[2] << " "
               << size[0] << " " << size[1] << " " << size[2] << " "
               << std::endl;

      for(int x = cent[2]-size[2]/2.0, x < cent[2]+size[2]/2.0, x++)
      {
          for(int y = cent[0]-size[0]/2.0, y < cent[0]+size[0]/2.0, y++)
          {
              int ind = x * kf->mImDep.cols + y;// 总索引
              cloud->points[ind].b = color_b[cluster.class_id];
              cloud->points[ind].g = color_g[cluster.class_id];
              cloud->points[ind].r = color_r[cluster.class_id];
          }
      }
    }
