...
ObjectDatabase::ObjectDatabase()
{
    ...
    // 不同颜色对应不同物体 ==========
    // 自己训练的数据集 10类物体======
    for (int i = 0; i < 11; i++) // 带有背景
    {  
       mvColors.push_back(cv::Scalar( i*10 + 40, i*10 + 40, i*10 + 40));
    }
    mvColors[0] = cv::Scalar(40,40,40);//background  bgr
    mvColors[1] = cv::Scalar(0,0,255);//mouse  红色
    mvColors[2] = cv::Scalar(0,255,0);//monitor 绿色
    mvColors[3] = cv::Scalar(0,255,255);//cup  黄色
    mvColors[4] = cv::Scalar(255,0,0);//chair  蓝色
    mvColors[5] = cv::Scalar(125,125,255);//fan
    mvColors[6] = cv::Scalar(255,125,125);//phone
    mvColors[7] = cv::Scalar(255,0,255);//box  品红色
    mvColors[8] = cv::Scalar(255,255,0);//keyboard 青色
    mvColors[9] = cv::Scalar(125,255,125);//person
    mvColors[10] = cv::Scalar(125,125,125);//bag
    ...
    mvSizes[0] = 0.6;
    mvSizes[1] = 0.1;   // mouse  0.06 米以内认为是同一个物体
    mvSizes[2] = 1;   // monitor
    mvSizes[3] = 0.2;   // cup
    mvSizes[4] = 0.5;    // chair
    mvSizes[5] = 0.3;    // fan
    mvSizes[6] = 0.1;   // phone
    mvSizes[7] = 0.4;    // box
    mvSizes[8] = 0.3;    // keyboard
    mvSizes[9] = 0.35;   // person
    mvSizes[10] = 0.4;   // bag
